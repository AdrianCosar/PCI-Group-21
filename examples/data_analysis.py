...
class Humans(Agent):
    ...
    def update(self) -> None:
        ...
        for agent,distance in self.in_proximity_accuracy():
            ...
            if distance <= 30: # first zone where they increase distance between them
                count+=1
                ...
                if move_away.length() > 0: #cant normalize a vector of 0
                    ...
                    if distance < 12:
                        ...
                        tester=move_away.normalize() * (12-distance)/2
            ...
            elif distance <= 60 and isinstance(agent, Diddler): #second zone where agents direction is the direction of the other agents
                move_away = self.pos-agent_pos #making negative vector of their postitions to move away
                if move_away.length() > 0: #cant normalize a vector of 0
            ...
            if 30<=distance <= 40 and isinstance(agent, Humans):
...
class Diddler(Agent):
    def update(self) -> None:
        ...
        cohesion = Vector2(0, 0)
        for agent,distance in self.in_proximity_accuracy():
            if distance <= 15 and isinstance(agent, Humans):
                agent.kill()
                child = self.reproduce()
                child.pos = self.pos + Vector2(10, 0)
                break
            agent_pos = agent.pos
            if distance <= 10 and isinstance(agent, Diddler): # first zone where they increase distance between them
                move_away = self.pos-agent_pos #making negative vector of their postitions to move away
                if move_away.length() > 0: #cant normalize a vector of 0
                    separation += move_away.normalize() #making it normalized so they all have the same weight while acting
            elif distance <= 50 and isinstance(agent, Humans):
                toward = agent_pos - self.pos # making a postive vector to move towards the other agents current location
                if toward.length() >0:
...
if __name__ == "__main__":
    print(
        ...
        Simulation(FlockingConfig(radius=160, seed=1,image_rotation=True))
        .batch_spawn_agents(
            100,
            Humans,
            images=[
